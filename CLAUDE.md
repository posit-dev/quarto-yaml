...
Releases are made by merging a version-bump PR: bump **both**
`[workspace.package] version` and the intra-workspace `quarto-yaml`
dependency version under `[workspace.dependencies]` in the root
`Cargo.toml`, open a PR, merge. CI (`.github/workflows/release.yml`) then
tests at the release commit, publishes both crates to crates.io in
dependency order via [Trusted
...
publish. A failed release run is safe to re-run (already-published crates
